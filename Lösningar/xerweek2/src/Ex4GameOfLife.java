...
public class Ex4GameOfLife extends Application {
    ...
    Cell[][] nextState(Cell[][] oldWorld) {
        ...
        for (int row = 0; row < size; row++) {
            for (int col = 0; col < size; col++) {
                ...
                newWorld[row][col] = applyRules(oldWorld[row][col], nAlive);
            }
        }
    ...
    Cell applyRules(Cell current, int nAlive) {
        if (current == Cell.ALIVE && (nAlive < 2 || 3 < nAlive)) {
            return Cell.DEAD;
        }
        if( current == Cell.DEAD && nAlive == 3){
            return Cell.ALIVE;
        }
        return current;
    }
